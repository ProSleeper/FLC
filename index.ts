...
const updateTile = (x: number, y: number) => {
    if ((map[y][x].isStone() || map[y][x].isFalling_stone()) && map[y + 1][x].isAir()) {
        map[y + 1][x] = new Falling_stone();
        map[y][x] = new Air();
    } else if ((map[y][x].isBox() || map[y][x].isFalling_box()) && map[y + 1][x].isAir()) {
...
const colorOfTile = (g: CanvasRenderingContext2D, x: number, y: number): void => {
    if (map[y][x].isFlux()) g.fillStyle = "#ccffcc";
    else if (map[y][x].isUnbreakable()) g.fillStyle = "#999999";
    else if (map[y][x].isStone() || map[y][x].isFalling_stone()) g.fillStyle = "#0000cc";
    else if (map[y][x].isBox() || map[y][x].isFalling_box()) g.fillStyle = "#8b4513";
